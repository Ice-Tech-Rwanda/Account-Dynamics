...
export default function AdminFaqsPage() {
  const { data, loading, search, setSearch, refresh } = useAdminList<any>({ endpoint: "/api/admin/faqs", pageSize: 20 });
  ...
  const columns: Column<any>[] = [
    { key: "question", label: "Question", sortable: true, render: (item) => <span className="font-medium truncate max-w-md">{item.question}</span> },
  ...
  return (
    <AdminPageShell title="FAQs" subtitle="Frequently asked questions" onRefresh={refresh} loading={loading} onAdd={() => setShowCreate(true)} addLabel="Add FAQ">
      <CrudDialog open={showCreate || !!editItem} onClose={() => { setShowCreate(false); setEditItem(null); }} onSave={handleSave} title={editItem ? "Edit FAQ" : "Add FAQ"} initial={editItem ?? {}} fields={[
