  ...
  return (
    <section className="relative min-h-[55vh] sm:min-h-[60vh] flex items-center overflow-hidden">
      <div className="absolute inset-0">
        <Image src="/events/open.jpg" alt="Rankings" fill className="object-cover" priority sizes="100vw" />
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-black leading-[1.06] tracking-[-0.03em] text-white">
              Player Rankings &{" "}
              <span className="text-accent">Standings</span>
            </h1>
            <p className="mt-4 text-sm sm:text-base text-slate-300 max-w-xl leading-relaxed">
              Official {siteConfig.name} player rankings based on tournament performance and rating points. Track your progress and compete for the top spot.
            </p>
          </motion.div>
          ...
          >
            <StatCounter label="Active Players" value={150} suffix="+" icon="Users" dark />
            <StatCounter label="Tournaments" value={45} suffix="+" icon="Trophy" dark />
            <StatCounter label="Rating Cap" value={1850} icon="Award" dark />
            <StatCounter label="Avg Rating" value={1580} icon="BarChart3" dark />
          </motion.div>
        </div>
      </div>
    </section>
